...
static void prvSetupTimerInterrupt( void )
{
	...
	#elif (defined TCE0_CCA_vect)  // Xmega Board Timer C0
		#define RT_VECT TCE0_OVF_vect
		TCE0_PERH = compare_match.byte[1];      // Set high byte of compare match
		TCE0_PERL = compare_match.byte[0];      // register, then low byte
		TCE0_CTRLA = (1 << TC0_CLKSEL0_bp);		// Prescale F_CPU / 1,
		TCE0_CTRLB = (1 << TC0_CCAEN_bp);		// Enable Compare Capture A
